...
def train(model, tokenizer, optimizer, scheduler, train_dataloader, dev_dataloader,  args):
    ...
    for epoch_i in range(0, epochs):
        ...
        if args.save:
            ...
            output_dir = os.path.join(args.save_dir,args.suff+"seed_"+str(args.SEED), "checkpoint-{}".format(epoch_i+1))
    ...
    if args.save:
        statsfile = os.path.join(args.save_dir,args.suff+"seed_"+str(args.SEED),"stats")
...
def run(args):
    ...
    #model_name_or_path = "microsoft/DialogRPT-updown"
    #if args.task=='train':
    ...
    special_tokens_dict = {'additional_special_tokens': ['[RELIGION]','[NAME]']}
    ...
    #print("Model config id is = ", model.config.eos_token_id)
    #print("Tokenizer config is = ", tokenizer.eos_token_id)
    #sys.exit()
    ...
    df_train = pd.read_csv(train_path, delimiter='\t', header=None, names=['sentence']).sample(frac=1, random_state=args.SEED)
